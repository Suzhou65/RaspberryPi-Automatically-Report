...
def ip_change_alert(disalbe_phone_book, mode_select=() ):
    report_time = auto.current_time()
    ip_configuration = auto.configuration(update_config=False)
    #Compare all
    if bool(mode_select) is False:
        list_64 = auto.get_address()
        ...
        if False in result_64:
            ...
            if bool(disalbe_phone_book) is True:
                auto.sending_alert(message_container,sender_account,sender_password,receiver_address,disalbe_phone_book=True)
                print(f"{report_time} | {result_64} | {ip_container4} | {ip_container6}")
            elif bool(disalbe_phone_book) is False:
                auto.sending_alert(message_container, disalbe_phone_book=False)
                print(f"{report_time} | {result_64} | {ip_container4} | {ip_container6}")
            ...
            auto.configuration(update_config)
        else:
    ...
    #Selection Compare
        ...
        #IPv4
        if mode_select == "IPv4":
            list_4 = auto.get_address(mode_select)
            ipv4_config = ip_configuration["ip_address_alert"]["ipv4"]
        ...
        #IPv6
            ...
            list_6 = auto.get_address(mode_select)
            ipv6_config = ip_configuration["ip_address_alert"]["ipv6"]
